...
// src/app/(dashboard)/dashboard/sales/orders/[id]/page.tsx
...
import type { SalesOrderRow, SalesOrderLine } from '@/lib/services/sales/sales-order.service';
      ...
      <table className="w-full text-sm">
        <thead className="bg-muted/50">
          <tr>
            ...
            <th className="px-3 py-2 text-right">Cant. Despachada</th>
        ...
        <tbody>
          {lines.map((l) => (
            <tr key={l.id} className="border-t">
              ...
              <td className="px-3 py-2 text-right tabular-nums">
                {parseFloat(l.qtyDelivered).toFixed(2)}
              </td>
...
export default function SalesOrderDetailPage() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    try {
      ...
      if (so.status === 'DRAFT') {
        ...
        setEditDeliveryDate(new Date(so.deliveryDate).toISOString().split('T')[0] ?? '');
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/v1/sales/orders/${orderId}`, {
        method: 'PUT',
        credentials: 'include',
  ...
  const handleConfirm = async () => {
    setConfirming(true);
    try {
      const res = await fetch(`/api/v1/sales/orders/${orderId}/confirm`, {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/sales/orders/${orderId}/create-invoice`, {
        method: 'POST',
        credentials: 'include',
      });
      const payload = (await res.json()) as {
  ...
  const handleCancel = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/sales/orders/${orderId}/cancel`, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
  ...
  const canInvoice = isConfirmed || isDelivered;
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">{order.customerName}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 sm:shrink-0">
          ...
          {isConfirmed && (
            <Button
              variant="outline"
              onClick={() => router.push(`/dashboard/sales/orders/${orderId}/deliver` as never)}
            >
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <p className="font-mono text-2xl font-bold text-green-600">{fmtL(order.total)}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm">{fmtDate(order.deliveryDate)}</p>
              </div>
              <div>
                <p className="text-muted-foreground text-xs font-medium tracking-wide uppercase">
                  Moneda
