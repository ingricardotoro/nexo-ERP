// src/app/api/v1/accounting/reports/cxp-aging/route.ts
...
/** GET /api/v1/accounting/reports/cxp-aging?asOfDate=YYYY-MM-DD */
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await agingReportService.getCxpAging(auth.companyId, asOfDate);
    return NextResponse.json({ success: true, data: report });
  } catch (error) {
    return handleApiError(error, 'GET /api/v1/accounting/reports/cxp-aging');
  }
}
